...
<div id="wrapper">
    ...
    <div id="container">
        ...
        <div id="rtcontent">
            ...
            <!--menulinks starts -->
            <div id="menulinks">
                <a class="nav-cat" href="index.php">All</a>
                <?php
                ...
                for ($x=1;$cat=mysqli_fetch_assoc($category);$x++){
                    ...
                    if ($x==$_GET['cat']){?> <a class="nav-cat active" href="category.php?cat=<?php echo $x ?>"><?php echo $cat['category_name']?></a>
            ...
            <?php
                ...
                $total_count_q=mysqli_query($connection,"SELECT COUNT(`id`) AS `counted` FROM `articles` WHERE `category`= " . (int) $_GET['cat']);
                ...
                if ($counted>$art_on_page){?>
                    <div class="paginator">
                        <?php if ($current_page>1){?>
                            <a class="prev" href="category.php?current_page=<?php echo ($current_page-1);?>&cat=<?php echo $_GET['cat'] ?>">&laquo; Previous</a>
                        <?php }
                        if ($max_pages>$current_page){?>
                            <a class="next" href="category.php?current_page=<?php echo ($current_page+1);?>&cat=<?php echo $_GET['cat'] ?>">Next &raquo;</a>
                        <?php } ?>
                    </div>
                <?php }
            $query=mysqli_query($connection,'SELECT * FROM `articles` WHERE `category`=' . (int) $_GET['cat'] . ' GROUP BY `date` DESC LIMIT ' . $offset .','. $art_on_page);
            while ($post=mysqli_fetch_assoc($query)){
                ...
                <div class="document">
                    ...
                    <div class="dochead">
                        <a class="title" href="details.php?id=<?php echo $post['id']?>&cat=<?php echo $cat_final?>" target="_blank" ><span class="dhead title"><?php echo $post['title']?></span></a>
                    </div>
                    <div>
                    ...
                    <div class="dcontent">
                        <?php if ($post['category']==1 or $post['category']==3) {
                            ...
                            ?><a class="read_more" href="details.php?id=<?php echo $post['id']?>&cat=<?php echo $cat_final?>" target="_blank">Читать далее</a>
                            <?php
                        }
